...
function Applications({ applications }) {
  ...
  const columns = [
    ...
    columnHelper.accessor("createjob.job_name", {
      cell: (info) => (
        isRowInEdit[info.row.index] ? (
          <input
            type="text"
            className="text-black"
            value={info.row.original.createjob.job_name}
            onChange={(e) => handleJobNameChange(info.row.index, e.target.value)}
          />
        ) : (
    ...
    columnHelper.accessor("applicant.user.name", {
      cell: (info) =>
        isRowInEdit[info.row.index] ? (
          <input
            type="text"
            className="text-black"
            value={info.row.original.applicant.user.name}
  ...
  const handleNameChange = (index, value) => {
    const newData = [...data];
    newData[index].applicant.user.name = value;
    setData(newData);
  };

  const handleJobNameChange = (index, value) => {
    const newData = [...data];
    newData[index].createjob.job_name = value;
    setData(newData);
  };
